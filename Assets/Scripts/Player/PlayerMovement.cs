...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            // Uvolnit ledge grab pokud hráč zmáčkne opačný směr → PUSH OFF
            if (horizontalInput != 0 && Mathf.Sign(horizontalInput) != lastFacingDirection)
            {
                ...
                wallJumpCooldownTimer = wallJumpCooldown; // sdílí cooldown s wall jumpem
        ...
        if (isGrabbingLedge)
        {
            ...
            // Snap hráče jednou se bezpečným horizontálním offsetem
            if (!hasSnapped)
            {
